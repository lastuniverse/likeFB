...
// где:
//	name - название списка рассылки (строка или regexp)
Messanger.prototype._test_middleware = function( name, middleware_name, flag ) {
	// возврат если name не существует или задан неверно
	// минимальный name '/' или //
	if ( !name ){
		console.log('1 false', name, middleware_name);
		return false;
	}

	// возврат true если name строка и соответствует текущему middleware_name
	if ( typeof name === 'string')
	if(flag && middleware_name.indexOf(name) + 1) {
		console.log('2 true', name, middleware_name);
		return true;
	}else if(!flag && middleware_name == name ) {
		console.log('3 true', name, middleware_name);
		return true;
	}
	...
	if ( Object.prototype.toString.call( name ) == "[object RegExp]" && name.test( middleware_name ) ){
		console.log('4 true', name, middleware_name);
		return true;
	}
	...
	if ( Object.prototype.toString.call( name ) == "[object Array]" ){
		for( let key of name ){
			if( !this._test_middleware(key, middleware_name,true) ){
				//console.log('4 false', key, middleware_name);
				return false;
			}
		}
		console.log('5 true', name, middleware_name);
		return true;
	}
